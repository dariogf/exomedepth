...
file_samples = Sys.getenv(c("file_samples"))
file_bed = Sys.getenv(c("bed_file"))
cat("samples:",file_samples,"\n")
cat("bed file:",file_bed,"\n")
...
vect=as.vector(samples$Samples)
cat("tipo as.vector(samples$Samples): ",typeof(vect),"\n")
cat("cVect:",summary(vect),"\n\n")
my.counts <- getBamCounts(bed.frame= mybed.hg19, bam.file=vect)
